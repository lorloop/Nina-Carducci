...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){const currentIndex=$(".gallery-item:visible").index(element);$(`#${lightboxId}`).data("currentIndex",currentIndex).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(lightboxId){const lightbox=$(`#${lightboxId}`);let currentIndex=lightbox.data("currentIndex");const items=$(".gallery-item:visible");currentIndex=(currentIndex-1+items.length)%items.length;lightbox.data("currentIndex",currentIndex);const newSrc=items.eq(currentIndex).attr("src");lightbox.find(".lightboxImage").attr("src",newSrc)},nextImage(lightboxId){const lightbox=$(`#${lightboxId}`);let currentIndex=lightbox.data("currentIndex");const items=$(".gallery-item:visible");currentIndex=(currentIndex+1)%items.length;lightbox.data("currentIndex",currentIndex);const newSrc=items.eq(currentIndex).attr("src");lightbox.find(".lightboxImage").attr("src",newSrc)},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
